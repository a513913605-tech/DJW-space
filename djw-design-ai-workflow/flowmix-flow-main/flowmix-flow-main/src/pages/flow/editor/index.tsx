...
import Inspector from '../../../components/EditorUI/Inspector';
...
const EditorPage = memo(() => {
  ...
  const activeNode = useMemo(() => {
    return data.nodes?.find((item: any) => item.id === curNodeId);
  }, [curNodeId, data.nodes]);

  const activeEdge = useMemo(() => {
    return data.edges?.find((item: any) => item.id === curEdgeId);
  }, [curEdgeId, data.edges]);
  ...
  const handleNewPage = useCallback(() => {
    const title =  'DJW Design AI Workflow';
  ...
  const handleAddAiNode = useCallback((template: { label: string; desc: string; tone: string }) => {
    ...
    addNode({
      ...
      data: {
        style: {
          text: {
            content: `${String(count).padStart(2, '0')} ${template.label}\n${template.desc}`,
    ...
    <div className={styles.flowWrap}>
      ...
      <main className={styles.flowContent}>
        <Panel 
          ...
          onAddNode={handleAddAiNode}
          onNewPage={handleNewPage}
          onDelPage={handleDelPage}
        >
        </Panel>
        <Inspector node={activeNode} edge={activeEdge} />
        <div 
          style={!isPreview ? {
            position: 'absolute', 
            top: 64, 
